...
우리가 생활하면서 숫자와 문자를 구분하듯이 컴퓨터도 데이터 타입을 통해 데이터를 구분합니다. 이러한 작업이 다른 프로그래밍 언어에서는 철저하지만 Python에서는 자동으로 데이터 타입을 정해주고 준다.
...
[python.org](https://www.python.org/downloads/)에 들어가서 가장 최근 버전의 Python을 다운받아서 설치한다. 리눅스는 따로 정리하지 않겠다.
...
Python 프로그램 작성을 도와주는 Python 전용 통합개발 환경이다. 실행을 시키는 방법은 2가지가 있다. 하나는 **IDLE 파일**을 직접적으로 여는 방법이다. 다음은 명령어로 여는 방법이다. **Mac Os**라면 **터미널(Terminal)**에서 **"python3"**이라고 입력하고, **Window**라면 **명령 프롬프트(cmd)**에서 **"python"**이라고 입력하면 된다.
...
지금 보이는 창은 인터랙티브 쉘이다. ~~용어는 중요하지 않다.~~ 이곳에서는 **Python의 코드를 바로 입력하여 바로 실행시킬 수 있다.**
...
입력과 저장을 했다면 **F5**을 눌러서 실행시켜보자. 그러면 인터랙티브 쉘에 우리가 작성한 소스코드가 실행된 것을 볼 수 있다.
...
프로그래밍 언어를 배울때 학생들이 처음으로 어렵다고 생각하는게 자료형 선언이다. 하지만 파이썬에서는 자료형을 따로 지정하지 않아도 자동으로 지정해 준다.

### Python의 철학

Python이 다른 언어와 다르게 간단하고 가독성이 좋은 것은 Python이 가지고 있는 철학 떄문이다. [python.org](https://www.python.org/dev/peps/pep-0020/) 또는 아래의 코드를 인터랙티브 쉘에서 입력을 하면 확인할 수 있다. 
